		...
		foreach($vhosts as $vhost)
		{
			if(is_dir('/var/www/'.$vhost) && file_exists('/etc/httpd/sites-enabled/'.$vhost.'.conf')) {
				...
				if(file_exists('/var/moodledata/'.$vhost)) {
					echo server_button($vhost, $php_version1, $port1).'</td><td>'.server_button($vhost, $php_version2, $port2).'</td><td>'.disable_button($vhost).'</td><td>'.cache_button($vhost);
				} else {
					echo server_button($vhost, $php_version1, $port1).'</td><td>'.server_button($vhost, $php_version2, $port2).'</td><td>'.disable_button($vhost);
				}
				echo "</td></tr>";
			}
		}
